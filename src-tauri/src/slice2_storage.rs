...
fn insert_segments(connection: &Connection, segments: &[SemanticSegment]) -> NativeResult<()> {
    for segment in segments {
        ...
        connection.execute(
            ...
            params![segment.id,segment.asset_id,segment.start,segment.end,segment.title,
                segment.description,segment.transcript_excerpt,observations,segment.importance,
                segment.suggested_decision,segment.confidence,segment.provenance],
        ).map_err(|error| error.to_string())?;
    }
    Ok(())
}
...
pub fn replace_local_analysis(
    ...
    transaction.execute("DELETE FROM analysis_frames WHERE asset_id=?1", [asset_id])
        .map_err(|error| error.to_string())?;
    transaction.execute(
        "DELETE FROM semantic_segments WHERE asset_id=?1 AND provenance='local'", [asset_id],
    ).map_err(|error| error.to_string())?;
    for frame in frames {
        transaction.execute(
            "INSERT INTO analysis_frames (id,asset_id,timestamp,path,reason,perceptual_hash)
             VALUES (?1,?2,?3,?4,?5,?6)",
            params![frame.id,frame.asset_id,frame.timestamp,frame.path,frame.reason,frame.perceptual_hash],
        ).map_err(|error| error.to_string())?;
    }
...
pub fn list_analysis_frames(project_path: &Path) -> NativeResult<Vec<AnalysisFrame>> {
    ...
    let rows = statement.query_map([], |row| Ok(AnalysisFrame {
        id: row.get(0)?,asset_id: row.get(1)?,timestamp: row.get(2)?,path: row.get(3)?,
        reason: row.get(4)?,perceptual_hash: row.get(5)?,
    })).map_err(|error| error.to_string())?;
...
pub fn list_semantic_segments(project_path: &Path) -> NativeResult<Vec<SemanticSegment>> {
    ...
    let rows = statement.query_map([], |row| {
        ...
        Ok(SemanticSegment {
            id: row.get(0)?,asset_id: row.get(1)?,start: row.get(2)?,end: row.get(3)?,
            title: row.get(4)?,description: row.get(5)?,transcript_excerpt: row.get(6)?,
            visual_observations: serde_json::from_str(&observations).unwrap_or_default(),
            importance: row.get(8)?,suggested_decision: row.get(9)?,confidence: row.get(10)?,
...
pub fn load_preview(project_path: &Path, preview_id: &str) -> NativeResult<RemoteRequestPreview> {
    ...
    let payload: String = connection.query_row(
        "SELECT payload_json FROM remote_request_previews WHERE id=?1", [preview_id], |row| row.get(0),
...
pub fn mark_preview_status(project_path: &Path, preview_id: &str, status: &str) -> NativeResult<()> {
    storage::open_database(project_path)?.execute(
        "UPDATE remote_request_previews SET status=?1 WHERE id=?2", params![status,preview_id],
    ).map(|_| ()).map_err(|error| error.to_string())
}

pub fn remote_analysis_approved(project_path: &Path) -> NativeResult<bool> {
    let count: i64 = storage::open_database(project_path)?.query_row(
        "SELECT COUNT(*) FROM remote_approvals WHERE scope='semantic-analysis'", [], |row| row.get(0),
    ).map_err(|error| error.to_string())?;
    Ok(count > 0)
}
...
pub fn approve_remote_analysis(project_path: &Path, approved_at: &str) -> NativeResult<()> {
    storage::open_database(project_path)?.execute(
        "INSERT OR REPLACE INTO remote_approvals (scope,approved_at)
         VALUES ('semantic-analysis',?1)", [approved_at],
    ).map(|_| ()).map_err(|error| error.to_string())
}
...
pub fn insert_message(project_path: &Path, value: &AssistantMessage) -> NativeResult<()> {
    storage::open_database(project_path)?.execute(
        "INSERT OR REPLACE INTO assistant_messages (id,conversation_id,role,content,created_at)
         VALUES (?1,?2,?3,?4,?5)",
        params![value.id,value.conversation_id,value.role,value.content,value.created_at],
    ).map(|_| ()).map_err(|error| error.to_string())
}
...
pub fn list_conversations(project_path: &Path) -> NativeResult<Vec<AssistantConversation>> {
    ...
    let rows = statement.query_map([], |row| Ok(AssistantConversation {
...
pub fn list_messages(project_path: &Path) -> NativeResult<Vec<AssistantMessage>> {
    ...
    let rows = statement.query_map([], |row| Ok(AssistantMessage {
...
pub fn save_plan(project_path: &Path, value: &EditPlan) -> NativeResult<()> {
    ...
    storage::open_database(project_path)?.execute(
        ...
        params![value.id,value.conversation_id,value.status,payload,value.created_at,value.updated_at],
    ).map(|_| ()).map_err(|error| error.to_string())
}

pub fn load_plan(project_path: &Path, plan_id: &str) -> NativeResult<EditPlan> {
    let payload: String = storage::open_database(project_path)?.query_row(
        "SELECT plan_json FROM edit_plans WHERE id=?1", [plan_id], |row| row.get(0),
...
pub fn list_plans(project_path: &Path) -> NativeResult<Vec<EditPlan>> {
    let connection = storage::open_database(project_path)?;
    let mut statement = connection.prepare(
        "SELECT plan_json FROM edit_plans ORDER BY updated_at",
    ).map_err(|error| error.to_string())?;
    let rows = statement.query_map([], |row| row.get::<_, String>(0))
        .map_err(|error| error.to_string())?;
    rows.map(|row| row.map_err(|error| error.to_string()).and_then(|payload|
        serde_json::from_str(&payload).map_err(|error| format!("Invalid saved edit plan: {error}"))
    )).collect()
}
...
pub fn empty_ai_job(project_path: &str, preview_id: &str, kind: &str) -> AiJob {
    AiJob {
        id: uuid::Uuid::new_v4().to_string(),project_path: project_path.into(),
        preview_id: preview_id.into(),kind: kind.into(),status: "queued".into(),
        stage: "Waiting to contact OpenRouter".into(),progress: 0.0,received_chars: 0,
        error: String::new(),result_plan_id: String::new(),
        started_at: chrono::Utc::now().to_rfc3339(),finished_at: None,
    }
}

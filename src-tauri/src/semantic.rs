...
fn frame_hash(source: &Path, timestamp: f64) -> Result<u64, String> {
    let output = Command::new("ffmpeg")
        .args(["-loglevel","error","-ss",&format!("{timestamp:.3}"),"-i"]).arg(source)
        .args(["-frames:v","1","-vf","scale=8:8,format=gray","-f","rawvideo","-"])
        .output().map_err(|_| "ffmpeg was not found while sampling semantic frames".to_string())?;
    ...
    Ok(pixels.iter().enumerate().fold(0, |hash, (index, value)|
        if u64::from(*value) >= average { hash | (1_u64 << index) } else { hash }
    ))
}

fn render_frame(source: &Path, timestamp: f64, output: &Path) -> Result<(), String> {
    let result = Command::new("ffmpeg")
        .args(["-loglevel","error","-y","-ss",&format!("{timestamp:.3}"),"-i"]).arg(source)
        .args(["-frames:v","1","-vf","scale=960:-2:force_original_aspect_ratio=decrease","-q:v","3"])
        .arg(output).output()
        .map_err(|_| "ffmpeg was not found while rendering semantic frames".to_string())?;
    if result.status.success() { Ok(()) } else { Err("FFmpeg could not render a semantic frame".into()) }
}
...
fn candidates(duration: f64, scenes: &[Scene]) -> Vec<(f64, String)> {
    ...
    if scenes.is_empty() {
        ...
        let mut time = 15.0;
        while time < duration { values.push((time, "Timed visual sample".into())); time += 15.0; }
    } else {
        for scene in scenes {
            values.push(((scene.start + 0.25).min(duration), format!("Visual change near {}", scene.label)));
            if scene.end - scene.start >= 5.0 {
                values.push((((scene.start + scene.end) / 2.0).min(duration),
    ...
    if values.len() <= MAX_FRAMES { return values; }
    (0..MAX_FRAMES).map(|index| {
        values[index * (values.len() - 1) / (MAX_FRAMES - 1)].clone()
    }).collect()
}

fn sample_frames(project_path: &Path, asset: &crate::models::MediaAsset, scenes: &[Scene])
    -> Result<Vec<AnalysisFrame>, String>
{
    if media::media_kind(Path::new(&asset.managed_path)) == MediaKind::Audio { return Ok(Vec::new()); }
    ...
    for (timestamp, reason) in candidates(asset.duration.max(0.0), scenes) {
        let Ok(hash) = frame_hash(Path::new(&asset.managed_path), timestamp) else { continue };
        if hashes.iter().any(|previous: &u64| (previous ^ hash).count_ones() <= 7) { continue; }
        let id = Uuid::new_v4().to_string();
        let path = directory.join(format!("{id}.jpg"));
        if render_frame(Path::new(&asset.managed_path), timestamp, &path).is_err() { continue; }
        hashes.push(hash);
        frames.push(AnalysisFrame {
            id,asset_id: asset.id.clone(),timestamp,path: path.to_string_lossy().into_owned(),
            reason,perceptual_hash: format!("{hash:016x}"),
        });
    }
    Ok(frames)
}

fn transcript_excerpt(asset_id: &str, start: f64, end: f64, transcript: &[TranscriptSegment]) -> String {
    transcript.iter().filter(|value|
        value.asset_id == asset_id && value.end >= start && value.start <= end
    ).map(|value| value.text.as_str()).collect::<Vec<_>>().join(" ")
}
...
fn local_segments(
    ...
    let mut ranges = scenes.iter().map(|scene| (scene.start,scene.end)).collect::<Vec<_>>();
    if ranges.is_empty() {
        ranges = transcript.iter().filter(|value| value.asset_id == asset.id)
            .map(|value| (value.start,value.end)).collect();
    }
    if ranges.is_empty() { ranges.push((0.0,asset.duration.max(0.0))); }
    let context_names = contexts.iter().take(4).map(|value| value.name.as_str())
        .collect::<Vec<_>>().join(", ");
...
pub fn rebuild_local_semantics(project_path: &Path, asset_ids: &[String])
    -> Result<Vec<SemanticSegment>, String>
{
    if asset_ids.is_empty() { return Err("Select at least one source".into()); }
    ...
    for asset_id in asset_ids {
        ...
        let scenes = all_scenes.iter().filter(|value| value.asset_id == asset_id.as_str())
...
fn truncate(value: &str, max: usize) -> String {
    ...
    format!("{}\n[truncated for this remote request]",value.chars().take(max).collect::<String>())
}
...
pub fn prepare_preview(
    ...
    let sources = asset_ids.iter().map(|id| {
        let asset = storage::find_asset(project_path,id)?;
        Ok(DisclosedSource { asset_id: asset.id,name: asset.name,duration: asset.duration })
    }).collect::<Result<Vec<_>,String>>()?;
    let frames = slice2_storage::list_analysis_frames(project_path)?.into_iter()
        .filter(|value| asset_ids.contains(&value.asset_id)).collect::<Vec<_>>();
    let semantic_segments = if kind == "semantic" { Vec::new() } else {
        slice2_storage::list_semantic_segments(project_path)?.into_iter()
            .filter(|value| asset_ids.contains(&value.asset_id)).collect()
    };
    let transcript = storage::list_transcript(project_path)?.into_iter()
        .filter(|value| asset_ids.contains(&value.asset_id)).take(MAX_TRANSCRIPT).collect::<Vec<_>>();
    let contexts = storage::list_contexts(project_path)?.into_iter().take(MAX_CONTEXTS)
        .map(|mut value| { value.content = truncate(&value.content,MAX_CONTEXT_CHARS); value })
        .collect::<Vec<_>>();
    let frame_bytes = frames.iter().map(|value| fs::metadata(&value.path).map(|m|m.len()).unwrap_or(0)).sum::<u64>();
    let text_bytes = serde_json::to_vec(&(&sources,&semantic_segments,&transcript,&contexts,&prompt))
        .map_err(|error| error.to_string())?.len() as u64;
    let preview = RemoteRequestPreview {
        id: Uuid::new_v4().to_string(),kind: kind.into(),model,asset_ids,sources,frames,
        semantic_segments,transcript,contexts,prompt,conversation_id,decision_id,
        estimated_bytes: frame_bytes+text_bytes,
        first_approval_required: kind == "semantic" && !slice2_storage::remote_analysis_approved(project_path)?,
...
#[tauri::command]
pub fn build_local_semantic_map(project_path: String, asset_ids: Vec<String>)
    -> Result<Vec<SemanticSegment>, String>
{
...
pub fn prepare_semantic_analysis(
    project_path: String,asset_ids: Vec<String>,state: State<'_,RuntimeState>,
) -> Result<RemoteRequestPreview,String> {
    let model = state.data.lock().map_err(|_|"Settings lock was poisoned".to_string())?
        .settings.openrouter_model.clone();
    prepare_preview(Path::new(&project_path),"semantic",asset_ids,
...
fn messages(preview: &RemoteRequestPreview) -> Result<Value,String> {
    ...
    for frame in &preview.frames {
        ...
        content.push(json!({"type":"image_url","image_url":{"url":frame_data_url(frame)?,"detail":"low"}}));
    }
    Ok(json!([
...
struct RemoteOutput { segments: Vec<RemoteSegment> }

#[derive(Deserialize)]
#[serde(rename_all="camelCase",deny_unknown_fields)]
struct RemoteSegment {
    asset_id:String,start:f64,end:f64,title:String,description:String,transcript_excerpt:String,
    visual_observations:Vec<String>,importance:String,suggested_decision:String,confidence:f64,
}

fn validate(project_path: &Path, preview: &RemoteRequestPreview, output: RemoteOutput)
    -> Result<Vec<SemanticSegment>,String>
{
    if output.segments.is_empty() { return Err("The model returned an empty semantic map".into()); }
    output.segments.into_iter().map(|value| {
        ...
        if !value.start.is_finite() || !value.end.is_finite() || value.start < 0.0
            || value.end < value.start || value.end > asset.duration+0.25
        ...
        { return Err(format!("The model returned invalid semantic data for {}",asset.name)); }
        Ok(SemanticSegment {
            id:Uuid::new_v4().to_string(),asset_id:value.asset_id,start:value.start,end:value.end,
            title:value.title,description:value.description,transcript_excerpt:value.transcript_excerpt,
            visual_observations:value.visual_observations,importance:value.importance,
            suggested_decision:value.suggested_decision,confidence:value.confidence,provenance:"both".into(),
        })
    }).collect()
}

#[tauri::command]
pub fn run_semantic_analysis(project_path:String,preview_id:String,approve_first:bool)
    -> Result<Vec<SemanticSegment>,String>
{
    let project_path=PathBuf::from(project_path);
    let preview=slice2_storage::load_preview(&project_path,&preview_id)?;
    if preview.kind!="semantic" { return Err("This disclosure is not a semantic-analysis request".into()); }
    if preview.first_approval_required && !slice2_storage::remote_analysis_approved(&project_path)? {
        if !approve_first { return Err("Approve the first remote semantic analysis before continuing".into()); }
    ...
    let response=openrouter::stream_structured_response(
        &preview.model,messages(&preview)?,"edentic_semantic_map",schema(),||false,|_|{}
    ).map_err(|error| { let _=slice2_storage::mark_preview_status(&project_path,&preview.id,"failed"); error })?;
...
mod tests {
    ...
    fn candidates_span_long_sources() {
        let scenes=(0..40).map(|index|Scene {
            id:index.to_string(),asset_id:"a".into(),start:f64::from(index)*10.0,
            end:f64::from(index+1)*10.0,label:index.to_string(),thumbnail_path:String::new(),
        }).collect::<Vec<_>>();
